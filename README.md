...
The whole survey (baseline through Result 4) is a true step-by-step wizard: every step's footer carries a consistent **Back**/**Next** control pair, right-aligned (`.pv-wiz-nav`, `wizNavHTML(step, nextLabel)` in `index.html`). Back is always present in the DOM (so the footer layout never shifts step-to-step) but hidden/disabled on the baseline step, which has no prior step. `prevStepFor(step)` is the single source of truth for what Back targets from any given step, including the branch where a photo upload skips the BCS step entirely (`state.baseline.bcsSkippedByPhoto` tracks which path was taken, so Back from Part 1 returns to the correct place). Next buttons keep each page's existing label and behavior unchanged (e.g. the photo-analysis branch on baseline, activity-minutes scoring on Part 1-4) - only their markup moved into the shared right-aligned group. Result pages 1-3 additionally keep a lower-emphasis "I'm happy with this estimate" link in the footer's left slot, a deliberate early-exit shortcut to the completion page that survives alongside the wizard controls. The completion page itself has no Back/Next - it's the finished-profile landing page, not part of the step-through survey.
...
Each Result page (1-4) shows a compact chart built from those two already-displayed numbers - a stack of thin progress bars (one per completed round, reusing the existing `.pv-bar` styling) plus a one-line trend sentence ("ticked up N points" / "held steady" / "First checkpoint logged.") and the answer count so far. Deliberately, this chart never surfaces a raw FI fraction or confidence percentage - same product rule as the rest of the result pages (see "Uncertainty is reflected in tone" above) - it's a visual layer over numbers already on screen, not a new disclosure.

Result 4 additionally has a small "View full decision breakdown (developer)" link, opening `renderDevBreakdown()` - a separate, explicitly technical page (not part of the wizard step sequence, no ring/Back-Next, just a Close button back to Result 4) showing the full decision trail table, each category's raw FI (0-1) alongside its 0-100 display score, a plain-text explanation of the equal-weighting-per-category mechanism (already documented in code comments, just surfaced here too), and the aggregation numbers (`observedFI`, `eFI` - the age/breed/size-expected FI baseline, now also returned from `computeResult()` - and the health multiplier) that the consumer-facing pages deliberately keep out of view.
...
- Sex/neuter status is **not** currently captured anywhere in the UI - `state.baseline.sexNeuter` exists as a dead default value only (a brief attempt to add a select input for it was tried and reverted); same as before the widget swap, this remains a data-capture gap, not a scoring one (see reproductive-status note further down).
